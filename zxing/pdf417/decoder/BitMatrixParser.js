...
class BitMatrixParser {
  ...
  this.MAX_ROW_DIFFERENCE = 6;
  this.MAX_ROWS = 90;
  //private static final int MAX_COLUMNS = 30;
  // Maximum Codewords (Data + Error)
  this.MAX_CW_CAPACITY = 929;
  this.MODULES_IN_SYMBOL = 17;

  this.rows = 0;
  //private int columns = 0;

  this.leftColumnECData = 0;
  this.rightColumnECData = 0;
  this.eraseCount = 0;
  this.erasure = null;
  this.ecLevel = -1;
   ...
   *         this row.
   */
  processRow(rowCounters, rowNumber, rowHeigh, codewords, next) {
    let width = this.bitMatrix.getWidth();
    let columnNumber = 0;
    let symbol = 0;
    for (let i = 0; i < width; i += this.MODULES_IN_SYMBOL) {
      for (let mask = this.MODULES_IN_SYMBOL - 1; mask >= 0; mask--) {
        if (rowCounters[i + (this.MODULES_IN_SYMBOL - 1 - mask)] >= (rowHeight >>> 1)) {
          symbol |= 1 << mask;
        }
      }
      ...
      if (columnNumber > 0) {
        ...
        if (cw < 0 && i < width - this.MODULES_IN_SYMBOL) {
        ...
        switch (rowNumber % 3) {
          ...
          case 2:
            ...
            if (rightColumnECData == leftColumnECData && leftColumnECData != 0) {
              this.ecLevel = parseInt(((rightColumnECData % 30) - rows % 3) / 3);
            }
            break;
        }
      }
  ...
  getCodeword(symbol) {
    let sym = symbol;
    sym &= 0x3ffff;
    let i = findCodewordIndex(sym);
    if (i == -1) {
      return -1;
    } else {
      let cw = this.CODEWORD_TABLE[i] - 1;
      cw %= 929;
      return parseInt(cw);
    }
  }
   ...
   */
  this.SYMBOL_TABLE = [0x1025e, 0x1027a, 0x1029e,
   ...
   */
  this.CODEWORD_TABLE = [2627, 1819, 2622, 2621, 1813,
